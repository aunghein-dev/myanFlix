      ...
      <div className="grid grid-cols-5 gap-x-6 sm:gap-x-10 gap-y-7 mt-5">
        {logos.map((logo) => (
          <Logo
            ...
            width={60}
            height={60}
            className="px-2 py-3 bg-white/90 rounded-xl hover:bg-white/80 transition-colors duration-300 ease-in-out cursor-pointer"
            color={logo.color}
          />
        ))}
      </div>
      ...
      <div className="flex flex-rows gap-x-6 mt-14 mb-20 sm:mb-7">
        {
          contact.map((logo) => (
            <Logo
              ...
              width={40}
              height={40}
              className="p-1 bg-white/90 rounded-xl hover:bg-white/80 transition-colors duration-300 ease-in-out cursor-pointer"
              color={logo.color}
            />
          ))
        }
